frappe.listview_settings["Cold Storage Dispatch Goods"] = {
  onload(listview) {
    listview.page.add_action_item(__("Bulk Approve"), () => {
      ...
      frappe.confirm(__("Approve {0} selected Dispatch Goods documents?", [names.length]), () => {
        frappe.call({
          method: "cold_storage.cold_storage.api.bulk_approve.bulk_approve",
          args: { doctype: "Cold Storage Dispatch Goods", names },
          freeze: true,
    ...
    listview.page.add_action_item(__("Bulk Reject"), () => {
      ...
      frappe.prompt([{ fieldname: "reason", label: __("Rejection Reason"), fieldtype: "Small Text", reqd: 1 }], (values) => {
        frappe.confirm(__("Reject {0} selected Dispatch Goods documents?", [names.length]), () => {
          frappe.call({
            method: "cold_storage.cold_storage.api.bulk_approve.bulk_reject",
            args: { doctype: "Cold Storage Dispatch Goods", names, reason: values.reason },
            freeze: true,
            callback: () => listview.refresh(),
          });
        });
