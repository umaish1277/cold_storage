frappe.pages['mobile-receipt-entry'].on_page_load = function (wrapper) {
			...
			<div class="form-section">
				...
				<div class="grid-row">
					...
					<div class="field-row">
						...
						<input type="tel" id="driver_phone" class="form-control" placeholder="0300-1234567">
					</div>
				</div>
			</div>
            ...
            var card = $(`
				<div class="item-card">
					...
					<div class="grid-row">
						...
						<div class="field-row">
							<label>Batch</label>
							<input type="text" class="form-control item-batch" data-index="${index}" value="${item.batch || ''}">
						</div>
					</div>
				</div>
			`).appendTo(container);
    ...
    $(wrapper).on('change', '.item-batch', function () {
        items[$(this).data('index')].batch = $(this).val();
    });
    ...
    $(wrapper).find('#submit-receipt-btn').click(() => {
        ...
        if (!data.customer || !data.warehouse || items.length === 0) {
            frappe.msgprint(__('Please fill all required fields and add at least one item.'));
            return;
        }

        frappe.dom.freeze(__('Creating Receipt...'));
        frappe.call({
            method: 'cold_storage.cold_storage.doctype.cold_storage_receipt.cold_storage_receipt.create_mobile_receipt',
            args: { data: data },
            callback: function (r) {
                frappe.dom.unfreeze();
                if (r.message) {
                    ...
                    $(wrapper).find('#vehicle_no, #driver_name, #driver_phone').val('');
                    items = [];
                    render_items();
                }
            }
